...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime, AWeapon* DamageCauser) {
	...
	for (auto& HitCharacter : HitCharacters) {
		if (HitCharacter == nullptr || DamageCauser == nullptr || FatherCharacter == nullptr) continue;
...
void ULagCompensationComponent::ProjectileServerScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime, AWeapon* DamageCauser) {
	...
	if (FatherCharacter && HitCharacter && Confirm.bHitConfirmed && DamageCauser) {
		const float Damage = Confirm.bHeadShot ? DamageCauser->GetHeadShotDamage() : DamageCauser->GetDamage();
...
FFramePackage ULagCompensationComponent::InterpBetweenFrames(const FFramePackage& Older, const FFramePackage& Younger, float HitTime) {
	...
	for (auto& YoungerPair : Younger.HitBoxInfo)
	{
		const FName& BoxInfoName = YoungerPair.Key;

		const FBoxInformation& OlderBox = Older.HitBoxInfo[BoxInfoName];
		const FBoxInformation& YoungerBox = Younger.HitBoxInfo[BoxInfoName];
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations) {
	...
	// Enable body but disable head hitboxes
	for (auto& Frame : FramePackages) {
